...
// Copyright (c) 2021 The Katmancoin developer
...
#if defined(NDEBUG)
# error "Katmancoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x9c2e7a9d0d38374a7174279afbfdff3e68ea39ff8b4537b5ab23a4983994129b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Katmancoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Katmancoin Signed Message:\n";
...
{
    // Katmancoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Katmancoin: three days
static const int64 nTargetSpacing = 2.5 * 60; // Katmancoin: two minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Katmancoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Katmancoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x81be8aaf750acdb56307df36e74b2095d4be4773767aea89b3a0684e58fdbbf6");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "07/03/2021 Katmancoin nuribette kullanılacak birim oldu...";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04c55d349e537bb91120f0e1b22f6e57b68ff991d56083018606c3fce92fddcf854f06438aa96ad8f6da588a2ef7e9f53210ca923e7a9e916dbc2e44b97d85867f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1615118084;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087151421;

        if (fTestNet)
        {
            block.nTime    = 1615118071;
            block.nNonce   = 386660049;
        }
	...
	if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x1b0adbf91f5e315e845510ef3250ab064d02cb9bb3530871252c5c1f19e0561c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfl, 0xc6, 0xb1, 0xdc }; // Katmancoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KatmancoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KatmancoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KatmancoinMiner(CWallet *pwallet)
{
    printf("KatmancoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("katmancoin-miner");
    ...
    try { loop {
        ...
        printf("Running KatmancoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KatmancoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KatmancoinMiner, pwallet));
}
